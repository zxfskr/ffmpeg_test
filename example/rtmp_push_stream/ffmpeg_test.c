...
static int read_ts(FILE *fp, unsigned int *timestamp){
    // 3字节timestamp + 1字节timestamp_ex， ex部分为最高位
    unsigned char ts[3] = {0,};
    unsigned char ts_ex = 0;
    if(fread(&ts, 1, 3, fp) !=3) {
        printf("Failed to read_ts!\n");
        return -1;
    }

    if(fread(&ts_ex, 1, 1, fp) !=1) {
        printf("Failed to read_ts!\n");
        return -1;
    }

    *timestamp = ts_ex * 0x1000000 + ts[0] * 0x10000 + \
        ts[1] * 0x100 + ts[2];
...
void publish_stream(){
    ...
    char *flv = "../../video/pikachu.flv";
    char *rtmpaddr = "rtmp://192.168.1.167/live/test";
